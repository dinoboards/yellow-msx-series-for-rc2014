...
#define writeSectorToDevice(driverSlot, deviceIndex, lunIndex, firstDeviceSector)                                                  \
  deviceSectorWrite(driverSlot, deviceIndex, lunIndex, firstDeviceSector)

uint8_t deviceSectorRead(uint8_t driverSlot, uint8_t deviceIndex, uint8_t lunIndex, uint32_t firstDeviceSector) {
  return msxdosDevRead(driverSlot, deviceIndex, lunIndex, firstDeviceSector, 1, sectorBuffer);
}
uint8_t deviceSectorWrite(uint8_t driverSlot, uint8_t deviceIndex, uint8_t lunIndex, uint32_t firstDeviceSector) {
  return msxdosDevWrite(driverSlot, deviceIndex, lunIndex, firstDeviceSector, 1, sectorBuffer);
}

void preparePartitioningProcess(uint8_t        _driverSlot,
...
static uint8_t calculateFatFileSystemParametersFat12(uint32_t fileSystemSizeInK, dosFilesystemParameters *parameters) {
  ...
  parameters->totalSectors =
      dataSectorsCount + 1 + (sectorsPerFat * FAT_COPIES) + (FAT12_ROOT_DIR_ENTRIES / DIR_ENTRIES_PER_SECTOR);
...
static uint8_t calculateFatFileSystemParametersFat16(uint32_t fileSystemSizeInK, dosFilesystemParameters *parameters) {
  ...
  parameters->totalSectors =
      dataSectorsCount + 1 + (sectorsPerFat * FAT_COPIES) + (FAT16_ROOT_DIR_ENTRIES / DIR_ENTRIES_PER_SECTOR);
  parameters->dataSectors             = dataSectorsCount;
  parameters->clusterCount            = clusterCount;
  parameters->sectorsPerFat           = sectorsPerFat;
  parameters->sectorsPerCluster       = sectorsPerCluster;
...
static uint8_t createFatFileSystem(
    uint8_t driverSlot, uint8_t deviceIndex, uint8_t lunIndex, uint32_t firstDeviceSector, uint32_t fileSystemSizeInK) {
