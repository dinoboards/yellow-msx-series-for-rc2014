...
FSS_MENU:
	DB	"\r\n"
	DB	"SELECT FLOPPY DISK CONTROLLER:\r\n"
	DB	"  (A) Disk IO ECB Board\r\n"
	DB	"  (B) Disk IO 3 ECB Board\r\n"
	DB	"  (C) Zeta SBC Onboard FDC\r\n"
	DB	"  (D) Zeta 2 SBC Onboard FDC\r\n"
	DB	"  (E) Dual IDE ECB Board\r\n"
	DB	"  (F) N8 Onboard FDC\r\n"
	DB	"  (G) RC2014 SMC (SMB)\r\n"
	DB	"  (H) RC2014 WDC (SMB)\r\n"
	DB	"  (I) SmallZ80 Expansion\r\n"
	DB	"  (J) Dyno-Card FDC, D1030\r\n"
	DB	"  (K) RC2014 EPFDC\r\n"
	DB	"  (X) Exit\r\n"
	DB	"=== OPTION ===> $\r\n"
;
...
;
MM_TABLE	DB	'S'
		DW	MMS_SETUP, MM_SETUP
MM_ENTSIZ	EQU	$ - MM_TABLE
		DB	'R'
		DW	MMS_READ, MM_READ
		DB	'W'
		DW	MMS_WRITE, MM_WRITE
		DB	'F'
		DW	MMS_FORMAT, MM_FORMAT
		DB	'V'
		DW	MMS_VERIFY, MM_VERIFY
		DB	'I'
		DW	MMS_INITBUF, MM_INITBUF
		DB	'D'
		DW	MMS_DUMPBUF, MM_DUMPBUF
		DB	'C'
		DW	MMS_FDCMENU, MM_FDCMENU
		DB	'X'
		DW	MMS_EXIT, 0000H
...
STR_MAINMENU:
	DB	"=======================<< FDU MAIN MENU >>======================\r\n"
;	DB	"(S)ETUP: UNIT=XX  MEDIA=XXXXXX	 MODE=XXXXXXXXXX  TRACE=XX\r\n"
	DB	"(S)ETUP: UNIT="
MV_UNIT	DB		      "XX"
	DB			"  MEDIA="
MV_MED	DB				"XXXXXX"
	DB				      "	 MODE="
MV_MODE	DB					     "XXXXXXXXXX"
	DB						       "  TRACE="
MV_TRC	DB							       "XX"
	DB								 "\r\n"
	DB	"----------------------------------------------------------------\r\n"
	DB	"(R)EAD		(W)RITE		(F)ORMAT	(V)ERIFY\r\n"
	DB	"(I)NIT BUFFER	(D)UMP BUFFER	FDC (C)MDS	E(X)IT\r\n"
	DB	"=== OPTION ===> $\r\n"
;
MMS_SETUP:	DB	"SETUP$"
MMS_READ:	DB	"READ$"
MMS_WRITE:	DB	"WRITE$"
MMS_FORMAT:	DB	"FORMAT$"
MMS_VERIFY:	DB	"VERIFY$"
MMS_INITBUF:	DB	"INITIALIZE BUFFER$"
MMS_DUMPBUF:	DB	"DUMP BUFFER$"
MMS_FDCMENU:	DB	"FDC MENU$"
MMS_EXIT:	DB	"EXIT$"
;
...
; MAIN MENU PROMPTS
;
MMP_FILL	DB	"FILL VALUE$"
MMP_PATTERN	DB	"PATTERN START VALUE$"
MMP_UNIT	DB	"UNIT$"
MMP_TRACE	DB	"TRACE LEVEL$"
MMP_TRACK	DB	"TRACK$"
MMP_HEAD	DB	"HEAD$"
MMP_SECTOR	DB	"SECTOR$"
MMP_MEDIA	DB	"MEDIA$"
MMP_MODE	DB	"MODE$"
MMP_INTRLV	DB	"INTERLEAVE$"
;
; MAIN MENU TARGET
;
...
;
MMTS_SECTOR	DB	"SECTOR$"
MMTS_TRACK	DB	"TRACK$"
MMTS_DISK	DB	"DISK$"
MMTS_RANDOM	DB	"RANDOM$"
;
; MAIN MENU DATA
;
...
;
MMPS_TGTRW	DB	"(S)ECTOR, (T)RACK, (D)ISK, (R)ANDOM ===> $"
MMPS_TGTF	DB	"(T)RACK, (D)ISK ===> $"
MMPS_IBOPT	DB	"(P)ATTERN, (F)ILL ===> $"
;
; MAIN MENU BUFFER OPTIONS
;
MMBS_PATTERN	DB	"PATTERN$"
MMBS_FILL	DB	"FILL$"
;
;
;
...
;
STR_CONTINUE:
	DB	"CONTINUE? (A)BORT, (R)ETRY, (I)GNORE ===> $"
;
DIO_SETMEDIA:
...
;
DIOCS_ABORT	DB	"ABORT$"
DIOCS_RETRY	DB	"RETRY$"
DIOCS_IGNORE	DB	"IGNORE$"
;
; DISK PROGRESS LABELS
;
DPL_TRACK	DB	"TRACK$"
DPL_HEAD	DB	"HEAD$"
DPL_SECTOR	DB	"SECTOR$"
;
; DISK OPERATIONS
;
...
FCB_PC720	DB	009H		; SECTOR COUNT
		...
		ENDIF
DTL_PC720	DB	"720KB $"
DTS_PC720	DB	"3.5\" 720KB - 9 SECTORS, 2 SIDES, 80 TRACKS, DOUBLE DENSITY$"
;
...
FCB_PC144	DB	012H		; SECTOR COUNT
		...
		ENDIF
DTL_PC144	DB	"1.44MB$"
DTS_PC144	DB	"3.5\" 1.44MB - 18 SECTORS, 2 SIDES, 80 TRACKS, HIGH DENSITY$"
;
...
FCB_PC320	DB	008H		; SECTOR COUNT
		...
		ENDIF
DTL_PC320	DB	"320KB $"
DTS_PC320	DB	"5.25\" 320KB - 8 SECTORS, 2 SIDES, 40 TRACKS, DOUBLE DENSITY$"
;
...
FCB_PC360	DB	009H		; SECTOR COUNT
		...
		ENDIF
DTL_PC360	DB	"360KB $"
DTS_PC360	DB	"5.25\" 360KB - 9 SECTORS, 2 SIDES, 40 TRACKS, DOUBLE DENSITY$"
;
...
FCB_PC120	DB	00FH		; SECTOR COUNT
		...
		ENDIF
DTL_PC120	DB	"1.2MB $"
DTS_PC120	DB	"5.25\" 1.2MB - 15 SECTORS, 2 SIDES, 80 TRACKS, HIGH DENSITY$"
;
...
FCB_PC111	DB	00FH		; SECTOR COUNT
		...
		ENDIF
DTL_PC111	DB	"1.11MB$"
DTS_PC111	DB	"8\" 1.11MB - 15 SECTORS, 2 SIDES, 77 TRACKS, DOUBLE DENSITY$"
;
;===============================================================================
...
FM_ENTSIZ	EQU	$ - FM_TABLE
		DB	'D'
		DW	FMS_READDEL, FM_READDEL
		DB	'W'
		DW	FMS_WRITE, FM_WRITE
		DB	'E'
		DW	FMS_WRITEDEL, FM_WRITEDEL
		DB	'T'
		DW	FMS_READTRK, FM_READTRK
		DB	'I'
		DW	FMS_READID, FM_READID
		DB	'F'
		DW	FMS_FMTTRK, FM_FMTTRK
		DB	'Q'
		DW	FMS_SCANEQ, FM_SCANEQ
		DB	'L'
		DW	FMS_SCANLOEQ, FM_SCANLOEQ
		DB	'H'
		DW	FMS_SCANHIEQ, FM_SCANHIEQ
		DB	'C'
		DW	FMS_RECAL, FM_RECAL
		DB	'N'
		DW	FMS_SENSEINT, FM_SENSEINT
		DB	'P'
		DW	FMS_SPECIFY, FM_SPECIFY
		DB	'V'
		DW	FMS_DRVSTAT, FM_DRVSTAT
		DB	'S'
		DW	FMS_SEEK, FM_SEEK
		DB	'O'
		DW	FMS_VERSION, FM_VERSION
		DB	'U'
		DW	FMS_PULSETC, FM_PULSETC
		DB	'A'
		DW	FMS_DOR, FM_DOR
		DB	'M'
		DW	FMS_MOTOR, FM_MOTOR
		DB	'Z'
		DW	FMS_FDCRESET, FM_FDCRESET
		DB	'X'
		DW	FMS_EXIT, 0000H
...
;
STR_FDCMENU:
;	DB	"===================<< FDU FDC COMMAND MENU >>======= [MSR=XX] ==\r\n"
	DB	"===================<< FDU FDC COMMAND MENU >>======= [MSR="
MV_MSR	DB								  "XX"
	DB								    "] ==\r\n"
	DB	"(R)EAD		 READ (D)EL	 (W)RITE	 WRITE D(E)L\r\n"
	DB	"READ (T)RK	 READ (I)D	 (F)ORMAT	 SCAN E(Q)\r\n"
	DB	"SCAN (L)O/EQ	 SCAN (H)I/EQ	 RE(C)AL	 SE(N)SE INT\r\n"
	DB	"S(P)ECIFY	 DRI(V)E STAT	 (S)EEK		 VERSI(O)N\r\n"
;	DB	"P(U)LSE TC	 L(A)TCH:XX	 (M)OTOR:XXX	 FDC RESET (Z)\r\n"
	DB	"P(U)LSE TC	 L(A)TCH:"
MV_LAT	DB				"XX"
	DB				  "	 (M)OTOR:"
MV_MOT	DB						"XXX"
	DB						   "	 FDC RESET (Z)"
...
FMS_NOP		DB	"NOP$"
FMS_READ	DB	"READ$"
FMS_READDEL	DB	"READ DELETED$"
FMS_WRITE	DB	"WRITE$"
FMS_WRITEDEL	DB	"WRITE DELETED$"
FMS_READTRK	DB	"READ TRACK$"
FMS_READID	DB	"READ ID$"
FMS_FMTTRK	DB	"FORMAT TRACK$"
FMS_SCANEQ	DB	"SCAN EQUAL$"
FMS_SCANLOEQ	DB	"SCAN LOW OR EQUAL$"
FMS_SCANHIEQ	DB	"SCAN HIGH OR EQUAL$"
FMS_RECAL	DB	"RECALIBRATE$"
FMS_SENSEINT	DB	"SENSE INTERRUPT$"
FMS_SPECIFY	DB	"SPECIFY$"
FMS_DRVSTAT	DB	"DRIVE STATUS$"
FMS_SEEK	DB	"SEEK$"
FMS_VERSION	DB	"VERSION$"
FMS_PULSETC	DB	"PULSE TC$"
FMS_DOR		DB	"DOR$"
FMS_MOTOR	DB	"MOTOR$"
FMS_FDCRESET	DB	"FDC RESET$"
FMS_EXIT	DB	"EXIT$"
;
...
FRB_R		DB	0
FRB_N		DB	0
		DS	10H		; ALLOWS EXTRA CHARACTERS TO BE RETREIEVED
FRB_SIZ		EQU	$-FRB
;
; FDC COMMANDS
;
...
FCS_NOP:		DB	"NOP$"
FCS_READ:		DB	"READ$"
FCS_READDEL:		DB	"READ DELETED$"
FCS_WRITE:		DB	"WRITE$"
FCS_WRITEDEL:		DB	"WRITE DELETED$"
FCS_READTRK:		DB	"READ TRACK$"
FCS_READID:		DB	"READ ID$"
FCS_FMTTRK:		DB	"FORMAT TRACK$"
FCS_SCANEQ:		DB	"SCAN EQUAL$"
FCS_SCANLOEQ:		DB	"SCAN LOW OR EQUAL$"
FCS_SCANHIEQ:		DB	"SCAN HIGH OR EQUAL$"
FCS_RECAL:		DB	"RECALIBRATE$"
FCS_SENSEINT:		DB	"SENSE INTERRUPT$"
FCS_SPECIFY:		DB	"SPECIFY$"
FCS_DRVSTAT:		DB	"DRIVE STATUS$"
FCS_SEEK:		DB	"SEEK$"
FCS_VERSION:		DB	"VERSION$"
;
; FDC COMMAND TABLE
;
FCT		DB	CMD_READ
		DW	FCS_READ,	FXRJ_READ
FCT_ENTSIZ	EQU	$ - FCT
		DB	CMD_READDEL
		DW	FCS_READDEL,	FXRJ_READDEL
		DB	CMD_WRITE
		DW	FCS_WRITE,	FXRJ_WRITE
		DB	CMD_WRITEDEL
		DW	FCS_WRITEDEL,	FXRJ_WRITEDEL
		DB	CMD_READTRK
		DW	FCS_READTRK,	FXRJ_READTRK
		DB	CMD_READID
		DW	FCS_READID,	FXRJ_READID
		DB	CMD_FMTTRK
		DW	FCS_FMTTRK,	FXRJ_FMTTRK
		DB	CMD_SCANEQ
		DW	FCS_SCANEQ,	FXRJ_SCANEQ
		DB	CMD_SCANLOEQ
		DW	FCS_SCANLOEQ,	FXRJ_SCANLOEQ
		DB	CMD_SCANHIEQ
		DW	FCS_SCANHIEQ,	FXRJ_SCANHIEQ
		DB	CMD_RECAL
		DW	FCS_RECAL,	FXRJ_RECAL
		DB	CMD_SENSEINT
		DW	FCS_SENSEINT,	FXRJ_SENSEINT
		DB	CMD_SPECIFY
		DW	FCS_SPECIFY,	FXRJ_SPECIFY
		DB	CMD_DRVSTAT
		DW	FCS_DRVSTAT,	FXRJ_DRVSTAT
		DB	CMD_SEEK
		DW	FCS_SEEK,	FXRJ_SEEK
		DB	CMD_VERSION
		DW	FCS_VERSION,	FXRJ_VERSION
...
FXRR4:	IN	A,(C)			; [12] GET MSR
	CP	$F0			; [07] BYTE READY?
	JR	NZ,FXRR5A		; [12/7] GET IT

	INC	C			; [04] POINT TO DATA PORT
	INI				; [16] (HL) := (C), HL++, B--
...
CHR_ESC		EQU	1BH
;
STR_DRIVERESET	DB	"RESET DRIVE...$"
STR_EXECUTE	DB	"EXECUTION$"
STR_OP		DB	"OPERATION$"
STR_FORMAT	DB	"FORMAT$"
STR_SENDCMD	DB	"SEND COMMAND$"
STR_GETRESULTS	DB	"GET RESULTS$"
STR_SEEKWAIT	DB	"SEEK WAIT$"
STR_DOR		DB	"SET DOR$"
STR_PROGRESS	DB	"PROGRESS$"
STR_MISMATCH	DB	"DATA MISMATCH AT $"
STR_RESET	DB	"FDCRESET...$"
STR_NOTIMPL	DB	"*** NOT IMPLEMENTED ***$"
STR_NORESP	DB	"*** DRIVE NOT RESPONDING ***$"
STR_EOD		DB	"<EOD>$"
STR_EMPTY	DB	"<EMPTY>$"
STR_TIMEOUT	DB	"<TIMEOUT>$"
STR_ARROW	DB	" -->$"
STR_ENTER	DB	"ENTER$"
STR_ON		DB	"ON $"
STR_OFF		DB	"OFF$"
STR_DRV720	DB	"720KB $"
STR_DRV144	DB	"1.44MB$"
STR_MODEPOLL	DB	"POLLING   $"
STR_MODEINT	DB	"INTERRUPT $"
STR_MODEDMA	DB	"DMA	   $"
STR_CC0		DB	" CC0=$"
STR_CC1		DB	" CC1=$"
STR_CC2		DB	" CC2=$"
