...
uint8_t scsi_write(const uint8_t number_of_sectors, const uint32_t sector_number, const uint8_t *const buffer) {
...
uint8_t usb_dev_write(const uint8_t  lun,
  ...
  for (uint8_t index = 0; index < number_sectors_to_write; index++) {
    if ((result = scsi_write(1, sector_number, buffer)) != CH_USB_INT_SUCCESS) {
      yprintf(65, " EW %x ", result);
      *number_of_sectors_written = index;
