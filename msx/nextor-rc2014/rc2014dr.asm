...
DRV_CF_STATE_STORE:
	...
	LD	A, (IX+WRKAREA.ST_PRESENT)
	OR	PRES_MS
	LD	(IX+WRKAREA.ST_PRESENT), A		; SET BIT FLAG TO INDICATE MSX-MUSIC ROM IS PRESENT
...
DEV_READ_ROM:
	...
	LD	(IX+WRKAREA.ST_RD_CNT), B
	LD	(IX+WRKAREA.ST_RD_CND_RQ), B
	LD	A, (DE)			; RETRIEVE SECTOR NUMBER  (ASSUMING LESS THEN 255 FOR MOMENT)
	LD	(IX+WRKAREA.ST_INDX_LW), A
	INC	DE
	LD	A, (DE)
	LD	(IX+WRKAREA.ST_INDX_HI),A
...
SKIP1:
	LD	(IX+WRKAREA.ST_DEST_LW), L
	LD	(IX+WRKAREA.ST_DEST_HI), H

DEV_RW_LP:
	CALL	SEC_CPY

	PUSH	HL
	LD	L, (IX+WRKAREA.ST_INDX_LW)		; INCREMENT SECTOR IDX
	LD	H, (IX+WRKAREA.ST_INDX_HI)
	INC	HL
	LD	(IX+WRKAREA.ST_INDX_LW), L
	LD	(IX+WRKAREA.ST_INDX_HI), H
	POP	HL
skip:

	LD	A, (IX+WRKAREA.ST_RD_CNT)	; MORE SECTORS TO GO?
	DEC	A
	LD	(IX+WRKAREA.ST_RD_CNT), A
	JR	NZ, DEV_RW_LP

	LD	B, (IX+WRKAREA.ST_RD_CND_RQ)	; LOAD SECTORED COPIED COUNT
...
SEC_CPY:
	...
	LD	C, (IX+WRKAREA.ST_INDX_LW)
	LD	B, (IX+WRKAREA.ST_INDX_HI)
	CALL	DIV16
