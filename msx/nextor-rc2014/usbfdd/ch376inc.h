...
/* Oringinal file at https://github.com/changleo828/BOSSEN_USB_FOR_WT/blob/master/code/CH376INC.H */
/* google translated */
...
/* USB single data packet, the maximum length of the data block, the length of the default buffer */

#define CH376_DAT_BLOCK_LEN 0x40

/* ************************************************ **************************************************** ***************** */
/* command code */
/* Some commands are compatible with CH375 chip, but the input data or output data may be partially different) */
/* A command sequence of operations consists of:
 * A command code (for the serial port mode, two synchronization codes are required before the command code),
 * Several input data (can be 0),
 * Generate an interrupt notification or several output data (can be 0), choose one of the two, if there is an interrupt
 * notification, there must be no output data, and if there is output data, no interrupt will be generated Only the
 * CMD01_WR_REQ_DATA command is an exception, the sequence includes: one command code, one output data, several input data
 * Command code naming rules: CMDxy_NAME
 * Where x and y are numbers, x indicates the minimum number of input data (bytes), y indicates the minimum output data
 * (bytes), if y is H, it indicates that an interrupt notification is generated, Some commands can read and write data blocks
 * of 0 to multiple bytes, and the number of bytes of the data block itself is not included in the above x or y */
/* This file will also provide a command code format compatible with the CH375 chip command code by default (that is, after
 * removing x and y). If you don't need it, you can defineNO_CH375_COMPATIBLE_prohibit */
...
/* Serial port mode: set the serial port communication baud rate (the default baud rate after power-on or reset is
   9600bps, selected by D4/D5/D6 pins)
...
/* 10H=SDO pin is disabled for interrupt output, tri-state output is disabled when SCS chip selection is invalid, 90H=SDO pin
 * is also used as interrupt request output when SCS chip selection is invalid */
...
#define CMD01_RD_USB_DATA0                                                                                                  \
  0x27 /* Read data block from current USB interrupt endpoint buffer or host endpoint receive buffer */
...
#define CMD10_WR_USB_DATA7 0x2B /* Device mode: write data block to the send buffer of USB endpoint 2 */
...
#define CMD20_WR_OFS_DATA 0x2E /* Write a data block to the specified offset address in the internal buffer */
/* input: offset, length, stream */

#define CMD10_SET_FILE_NAME 0x2F /* Host file mode: set the filename of the file to be operated on */
/* Input: 0-terminated string (no more than 14 characters including terminator 0) */

/* ************************************************ **************************************************** ***************** */
/* Main command (Manual 1), commonly used, the following commands always generate an interrupt notification at the end of the
 * operation, and there is always no output data */

#define CMD0H_DISK_CONNECT 0x30 /* host file mode / SD card not supported: check if disk is connected */
/* output interrupt */

#define CMD0H_DISK_MOUNT 0x31 /* host file mode: initialize disk and test if disk is ready */
/* output interrupt */

/* Host file mode : open files or directories(folders), or enumerate files and directories(folders)
 * output interrupt   */
#define CMD0H_FILE_OPEN 0x32

#define CMD0H_FILE_ENUM_GO 0x33 /* Host file mode: continue enumerating files and directories (folders) */
/* output interrupt */
...
/* Input: Specify the index number of the directory information structure to be read in the sector */
/* The range of the index number is 00H~0FH, and the index number 0FFH is the currently opened file */
/* output interrupt */

#define CMD0H_DIR_INFO_SAVE 0x38 /*Host file mode : save file directory information */ /* output interrupt */
...
/* Input: The number of bytes requested to be read (total length is 16 bits, low byte first) */
/* output interrupt */
...
/* Input: The number of bytes requested to be written (total length is 16 bits, low byte first) */
/* output interrupt */
...
#define CMD1H_SEC_READ 0x4B /* host file mode / SD card not supported: read data blocks from current location in sectors */
/* input: the number of sectors requested to be read */
/* output interrupt */

#define CMD1H_SEC_WRITE 0x4C /* host file mode/SD card not supported: write data block at current location in sectors */
...
#define CMD11_GET_DEV_RATE 0x0A /* Host mode: Get the data rate type of the currently connected USB device */
/* input: data 07H */
...
/* output: status (USB_INT_CONNECT or USB_INT_DISCONNECT or USB_INT_USB_READY, other values ​​indicate that the operation
 * is not complete) */
...
#define CMD10_SET_ENDP3 0x19 /* Device mode (serial port not supported): set the transmitter of USB endpoint 0 */
/* input: how it works */
/* If bit 7 is 1, bit 6 is the synchronization trigger bit, otherwise the synchronization trigger bit remains unchanged */
...
#define CMD10_SET_ENDP5 0x1B /* Device mode (serial port not supported): set the transmitter of USB endpoint 1 */
/* input: how it works */
/* If bit 7 is 1, bit 6 is the synchronization trigger bit, otherwise the synchronization trigger bit remains unchanged */
/* Bit 3~Bit 0 is the transaction response mode: 0000~1000-Ready ACK, 1110-Busy NAK, 1111-Error STALL */
...
/* input: how it works */
/* If bit 7 is 1, bit 6 is the synchronization trigger bit, otherwise the synchronization trigger bit remains unchanged */
/* Bit 3~Bit 0 is the transaction response mode: 0000-ready ACK, 1101-ready but not returning ACK, 1110-busy NAK, 1111-error
 * STALL */

#define CMD10_SET_ENDP7 0x1D /* Set the transmitter for USB endpoint 2/host endpoint */
/* input: how it works */
/* If bit 7 is 1, bit 6 is the synchronization trigger bit, otherwise the synchronization trigger bit remains unchanged */
/* Bit 3~Bit 0 is the transaction response mode: 0000-ready ACK, 1101-ready but no response, 1110-busy NAK, 1111-error STALL
 */
...
/* ************************************************ **************************************************** ***************** */
/* Auxiliary commands (manual 2), not commonly used or for compatibility with CH375 and CH372, the following commands always
 * generate an interrupt notification at the end of the operation, and always have no output data */
 ...
 * trigger bit of the host endpoint OUT, bit 5~bit 0 must be 0 */
...
#define CMD0H_DISK_RESET 0x52 /* host mode/SD card not supported: control transfer - reset usb storage */
/* output interrupt */

#define CMD0H_DISK_SIZE 0x53 /* Host mode/SD card not supported: Get the capacity of USB storage */
/* output interrupt */

#define CMD0H_DISK_INQUIRY 0x58 /* host mode/SD card not supported: query USB storage characteristics */
/* output interrupt */
...
#define CMD0H_RD_DISK_SEC 0x5B /* Host file mode: read one sector of data from disk into internal buffer */
/* output interrupt */

#define CMD0H_WR_DISK_SEC 0x5C /* Host file mode: write data of one sector of internal buffer to disk */
/* output interrupt */

#define CMD0H_DISK_MAX_LUN 0x5D /* Host mode: control transfer - get the maximum logical unit number of USB storage */
/* output interrupt */
...
/* The following status codes are special event interrupts, if the USB bus suspend check is enabled through
 * CMD20_CHK_SUSPEND, then the interrupt status of USB bus suspend and sleep wakeup must be handled */
...
/* Only need to process in built-in firmware mode: USB_INT_EP1_OUT, USB_INT_EP1_IN, USB_INT_EP2_OUT, USB_INT_EP2_IN */
/* bit 7 - bit 4 is 0000 */
...
/* Bit 3-Bit 0 indicates the response from the USB device when communication failed: 0010=ACK, 1010=NAK, 1110=STALL,
 * 0011=DATA0, 1011=DATA1, XX00=timeout */
...
/* The following status code 1XH is used for the operation status code of USB host mode */
#ifndef CH_USB_INT_SUCCESS

/* USB transaction or transfer operation succeeded */
#define CH_USB_INT_SUCCESS 0x14

/* A USB device connection event is detected, it may be a new connection or reconnection after disconnection */
#define CH_USB_INT_CONNECT 0x15

/* USB device disconnection event detected */
#define CH_USB_INT_DISCONNECT 0x16

/* The data transmitted by USB is wrong or the buffer overflows due to too much data */
...
typedef struct FAT_DIR_INFO {
  ...
  UINT8  DIR_NTRes;              /* 0CH */
  UINT8  DIR_CrtTimeTenth;       /* 0DH, the time of file creation, counted in 0.1 second units */
...
typedef struct BULK_ONLY_CBW {
  UINT32 CBW_Sig;
  UINT32 CBW_Tag;
  UINT8 CBW_DataLen0; /* 08H, input: data transmission length, the valid value for input data is 0 to 48, and the valid value
                         for output data is 0 to 33 */
  UINT8  CBW_DataLen1;
  UINT16 CBW_DataLen2;
  UINT8  CBW_Flag; /* 0CH, input: transmission direction and other flags, if bit 7 is 1, input data, if bit 0, output data or
                      no data */
  UINT8 CBW_LUN;
...
typedef union CH376_CMD_DATA {
  ...
  /* CMD0H_DISK_MOUNT: Initialize the disk and test if the disk is ready, when executed for the first time */
  ...
  struct {
    UINT8 mUpdateFileSz; /* Input parameter: whether to allow the file length to be updated, or 0 to prohibit the update of
                            the length */
  } FileCLose;           /* CMD1H_FILE_CLOSE: close the currently opened file */

  struct {
    UINT8 mDirInfoIndex; /* Input parameters: specify the index number of the directory information structure to be read in
                            the sector, 0FFH is the currently opened file */
  } DirInfoRead;         /* CMD1H_DIR_INFO_READ: read the directory information of the file */

  union {
    UINT32 mByteOffset; /* Input parameter: number of offset bytes, offset in bytes (total length 32 bits, low byte first) */
    UINT32 mSectorLba; /* Return: the absolute linear sector number corresponding to the current file pointer, 0FFFFFFFFH has
                          reached the end of the file (total length 32 bits, low byte first) */
  } ByteLocate;        /* CMD4H_BYTE_LOCATE: move the current file pointer in bytes */

  struct {
    UINT16
    mByteCount; /* Input parameter: the number of bytes requested to be read (total length is 16 bits, low byte first) */
  } ByteRead;   /* CMD2H_BYTE_READ: read data block from current position in bytes */

  struct {
    UINT16 mByteCount; /* Input parameter: the number of bytes requested to be written (total length is 16 bits, low byte
                          first) */
  } ByteWrite;         /* CMD2H_BYTE_WRITE: Write a block of data to the current location in bytes */

  union {
    UINT32 mSectorOffset; /* Input parameter: number of offset sectors, offset in sector (total length 32 bits, low byte
                             first) */
    UINT32 mSectorLba; /* Return: the absolute linear sector number corresponding to the current file pointer, 0FFFFFFFFH has
                          reached the end of the file (total length 32 bits, low byte first) */
  ...
  struct {
    UINT8 mSectorCount; /* Input parameters: the number of sectors requested to be read */
                        /* return: the number of sectors allowed to be read */
    UINT8  mReserved1;
    UINT8  mReserved2;
    UINT8  mReserved3;
    UINT32 mStartSector; /* Return: The starting absolute linear sector number of the sector block allowed to be read (total
                            length 32 bits, low byte first) */
  } SectorRead;          /* CMD1H_SEC_READ: Read the data block from the current position in sectors */

  struct {
    UINT8 mSectorCount; /* Input parameters: the number of sectors requested to be written */
                        /* return: the number of sectors allowed to be written */
    UINT8  mReserved1;
    UINT8  mReserved2;
    UINT8  mReserved3;
    UINT32 mStartSector; /* Return: the starting absolute linear sector number of the sector block allowed to be written
                            (total length 32 bits, low byte first) */
  } SectorWrite;         /* CMD1H_SEC_WRITE: Write a data block at the current location in sectors */

  struct {
    UINT32 mDiskSizeSec; /* Return: the total number of sectors of the entire physical disk (total length 32 bits, low byte
                            first) */
  } DiskCapacity;        /* CMD0H_DISK_CAPACITY: Query the physical capacity of the disk */

  struct {
    UINT32 mTotalSector; /* Return: the total number of sectors of the current logical disk (total length is 32 bits, low
                            byte first) */
    UINT32 mFreeSector;  /* Return: the number of remaining sectors of the current logical disk (total length is 32 bits, low
                            byte first) */
    UINT8 mDiskFat;      /* Return: FAT type of the current logical disk, 1-FAT12, 2-FAT16, 3-FAT32 */
  } DiskQuery;           /* CMD_DiskQuery, query disk information */

  BULK_ONLY_CBW DiskBocCbw; /* Input parameters: CBW command structure */
  /* CMD0H_DISK_BOC_CMD: command to execute BulkOnly transfer protocol to USB memory */

  struct {
    UINT8 mMaxLogicUnit; /* return: the maximum logic unit number of the USB memory */
  ...
  struct {
    UINT32 mDiskSizeSec; /* Return: the total number of sectors of the entire physical disk (total length is 32 bits, high
                            byte first) */
  } DiskSize;            /* CMD0H_DISK_SIZE: Get the capacity of the USB memory */

  struct {
    UINT32 mStartSector; /* Input parameters: LBA sector address (total length 32 bits, low byte first) */
    UINT8  mSectorCount; /* Input parameters: the number of sectors requested to be read */
 ...
 * Bit 6 is used to indicate the subclass SubClass-Code of the USB storage device. If bit 6 is 0, it means that the subclass
 * is 6, and if bit 6 is 1, it means that the subclass is other than 6
 * Bit 5 is used to indicate the USB configuration status in USB device mode and the USB device connection status in USB host
 * mode
 * In USB device mode, if bit 5 is 1, the USB configuration is complete, and if bit 5 is 0, it has not yet been configured
 * In USB host mode, if bit 5 is 1, there is a USB device on the USB port, and if bit 5 is 0, there is no USB device on the
 * USB port
 * Bit 4 is used to indicate the buffer lock state in USB device mode. If bit 4 is 1, it means that the USB buffer is in a
 * locked state. If bit 6 is 1, it means it has been released
 * Other bits, reserved, do not modify */
#define CH_VAR_SYS_BASE_INFO 0x20

/* The number of retries for the USB transaction operation
 * If bit 7 is 0, it will not retry when receiving NAK, if bit 7 is 1, if bit 6 is 0, it will retry infinitely when receiving
 * NAK (you can use CMD_ABORT_NAK command to give up retry), if bit 7 is 1, bit 6 is 1 Retry for up to 3 seconds when NAK is
 * received
 * Bit 5~Bit 0 is the number of retries after timeout */
#define CH_VAR_RETRY_TIMES 0x25

/* Bit flag in host file mode
 * Bit 1 and bit 0, FAT file system flag of the logical disk, 00-FAT12, 01-FAT16, 10-FAT32, 11-Illegal
 * Bit 2, whether the FAT table data in the current buffer has been modified, 0-unmodified, 1-modified
 * Bit 3, the file length needs to be modified flag, the current file is appended with data, 0-no modification is required if
 * it is not appended, 1-modification is required if it has been appended
 * Other bits, reserved, do not modify */
 ...
 * bit 1, auto-identification, 0-SD card, 1-MMC card
 * Bit 2, automatic identification, 0-standard capacity SD card, 1-high capacity SD card (HC-SD)
 ...
 * Bit 7~Bit 4, the current logical unit number of the USB storage device, after CH376 initializes the USB storage device,
 * the default is to access the 0# logical unit
...
/* The sector offset of the current file pointer in the cluster, if it is 0xFF, it points to the end of the file and the end
 * of the cluster */
#define CH_VAR_CLUS_SEC_OFS 0x3C

/* 32-bit/4-byte variable
 * For FAT16 disk, it is the number of sectors occupied by the root directory, and for FAT32 disk, it is the starting cluster
 * number of the root directory (total length 32 bits, low byte first) */
#define CH_VAR_DISK_ROOT 0x44

/* The total number of clusters of the logical disk(total length is 32 bits, low byte first) */
#define CH_VAR_DSK_TOTAL_CLUS 0x48

/* The starting absolute sector number LBA of the logical disk(total length is 32 bits, low byte first) */
#define CH_VAR_DSK_START_LBA 0x4C

/* The starting LBA of the data area of ​​the logical disk (total length is 32 bits, low byte first) */
#define CH_VAR_DSK_DAT_START 0x50

/* The LBA corresponding to the data in the current disk data buffer (total length is 32 bits, low byte first) */
#define CH_VAR_LBA_BUFFER 0x54

/* The current read and write disk start LBA address (total length 32 bits, low byte first) */
#define CH_VAR_LBA_CURRENT 0x58

/* LBA address of the sector where the current file directory information is located (total length 32 bits, low byte first)
 */
#define CH_VAR_FAT_DIR_LBA 0x5C

/* The starting cluster number of the current file or directory (folder) (total length 32 bits, low byte first) */
#define CH_VAR_START_CLUSTER 0x60

/* The current cluster number of the current file (total length is 32 bits, low byte first) */
...
/* The current file pointer, the byte offset of the current read and write position(total length 32 bits, low byte first) */
