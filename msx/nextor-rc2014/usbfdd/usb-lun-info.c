...
typedef uint8_t (*usb_lun_info_driver)(storage_device_config *const dev, const uint8_t lun, nextor_lun_info *const info);

static uint8_t no_driver(storage_device_config *const dev, const uint8_t lun, nextor_lun_info *const info) {
  (void)dev;
  (void)lun;
  (void)info;
  return 1;
