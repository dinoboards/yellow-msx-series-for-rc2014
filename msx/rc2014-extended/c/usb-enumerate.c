...
usb_error get_config_descriptor(const uint8_t config_index, const uint8_t device_address, uint8_t *const buffer) {
  ...
  CHECK(hw_get_config_descriptor((config_descriptor *)buffer, config_index, sizeof(config_descriptor), device_address), x_printf("a %02x\r\n", result));
  // logConfig((config_descriptor *)buffer);

  CHECK(hw_get_config_descriptor((config_descriptor *)buffer, config_index, ((config_descriptor *)buffer)->wTotalLength, device_address), x_printf("b %02x\r\n", result));
...
usb_error op_parse_endpoint(_working *const working) __z88dk_fastcall {
  ...
  case USB_IS_HUB:
    working->state->hub_endpoint.number = endpoint->bEndpointAddress;
    working->state->hub_endpoint.toggle = 0;
    working->state->hub_endpoint.max_packet_size = endpoint->wMaxPacketSize;
    break;
  }
...
usb_error op_capture_driver_interface(_working *const working) __z88dk_fastcall {
  ...
  case USB_IS_MASS_STORAGE: {
    ...
    CHECK(hw_set_configuration(/*dev_cfg*/ NULL), x_printf("err %d\r\n", result));
    break;
  }

  case USB_IS_HUB:
    working->state->hub_config.interface_number = interface->bInterfaceNumber;
    working->state->hub_config.max_packet_size = working->desc.bMaxPacketSize0;
    working->state->hub_config.value = working->config.desc.bConfigurationvalue;
    working->state->hub_config.address = working->state->next_device_address++;
...
usb_error op_get_config_descriptor(_working *const working) __z88dk_fastcall {
  ...
  CHECK(get_config_descriptor(working->config_index, working->state->next_device_address, working->config.buffer), x_printf("XXX %d\r\n", result));
